...
--------|---------|------
2026-07-10 | — | تعديل التحقق من تداخل الاشتراكات (Date range overlap)
...
### تعديل التحقق من تداخل الاشتراكات (Date range overlap)

**الوصف:** تغيير منطق التحقق من الاشتراكات من "هل يوجد اشتراك فعال من نفس النوع" إلى "هل توجد أيام/فترات متداخلة حقيقياً".

**التغييرات:**

1. **`subscriptionService.js:hasActiveSameTypeSubscription()`** — تقبل `options.dates` (DAILY) أو `options.startDate/endDate` (WEEKLY):
   - **DAILY**: تقارن التواريخ الجديدة مع `DailyExecutionDate` للمشتركين النشطين
   - **WEEKLY**: تقارن المدى الجديد مع `startDate/endDate` للاشتراكات النشطة (`newStart <= oldEnd AND newEnd >= oldStart`)
   - بدون تواريخ: سلوك احتياطي (يبحث عن أي تواريخ مستقبلية)

2. **`cart.js`** — يستخرج `computedDates` من `itemData` للـ DAILY، أو يحسب المدى من `weeksCount` للـ WEEKLY

3. **`cartApprovals.js`** — نقل التحقق بعد حساب التواريخ، يتحقق بالتداخل الحقيقي

4. **`studentPortal.js`** — نقل التحقق بعد حساب `resolveDailyExecutionDates()`

**رسائل الخطأ الجديدة:**
- DAILY: `'لديك اشتراك يومي في أحد هذه الأيام، يرجى مراجعة التواريخ المختارة'`
- WEEKLY: `'لديك اشتراك أسبوعي يتداخل مع فترة الاشتراك المطلوبة'`

**اختبارات (14 حالة):**
- ✅ DAILY: سبت + أحد = يسمح
- ✅ DAILY: أسبوع كامل + يوم خارج الأسبوع = يسمح
- ✅ DAILY: [10,11,12] + [13] = يسمح
- ❌ DAILY: سبت + سبت = يرفض
- ❌ DAILY: [12] + [12] = يرفض
- ❌ DAILY: [10,11,12] + [12,13] = يرفض
- ✅ DAILY: [10,11] + [13,14] = يسمح
- ✅ WEEKLY: يوليو + أغسطس = يسمح
- ❌ WEEKLY: 1-31 يوليو + 15 يوليو-15 أغسطس = يرفض
- ✅ WEEKLY: يوليو + سبتمبر = يسمح
- ✅ WEEKLY: 1-15 يناير + 20-31 يناير = يسمح
- ❌ WEEKLY: نفس المدى = يرفض
- ❌ WEEKLY: 20 يوليو-20 أغسطس + 1-31 يوليو = يرفض
- ✅ WEEKLY: 1-31 يوليو + 1-31 أغسطس = يسمح (حدود متجاورة)

**التصنيف:** منطق أعمال
**Backward Compatibility:** نعم — السلوك الجديد أقل تشدداً (يسمح باشتراكات جديدة ما لم يوجد تداخل حقيقي)
